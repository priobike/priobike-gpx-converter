...
def main():
    ...
    last_approx_points = inf
    for angle in range(0, 5):
        last_approx_points = inf
        for dt1 in range(1, 10):
            last_approx_points = inf
            for dt2 in range(1, 10):
                # approximated waypoints
                a_thrsh = angle * (1/10) * pi
                d1_thrsh = dt1 * 1000
                d2_thrsh = dt2 * 1000
                ...
                for i in range(len(rec_xs)):
                    ...
                    total_d += d
                total_d = total_d / len(rec_xs)

                print(f'#gpx points: {len(gpx_xs)}, #approx points: {len(approx_xs)}, a_thrsh: {a_thrsh}, d_thrsh1: {d1_thrsh}, d_thrsh2: {d2_thrsh}, d: {total_d}')
                # if len(approx_xs) == 2: break
                # if last_approx_points == len(approx_xs): break
                # if total_d > 200: break
                last_approx_points = len(approx_xs)
    # plt.show()
